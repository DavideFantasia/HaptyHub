...
Promise.all(cleanedBatch.map(graph => elk.layout(graph)))
   .then(outputBatch => {
       ...
       outputBatch.forEach(graph => {
           graph.screen_width = 135.0; // change back to 135.0 for Samsung Tablet
           graph.screen_height = 217.0; // change back to 217.0 for Samsung Tablet
       });
       // ------------------------------------------
